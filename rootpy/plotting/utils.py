...
class Compare(object):
    ...
    func : string, optional (default=None)
         ...
         as variable names (x0,x1,x2...), to access errors use
         the letter 'd' in front of variable names (dx0,dx1,dx2...).
    ...
    Example
    -------
        comp =  Compare({'x1':[h10,h11],'x2':[h20,h21,h22]}, '(x1-x2)/x1')

    This will calculate:
        (h10-h20)/h10, (h10-h21)/h10, (h10-h22)/h10, (h11-h20)/h11,
        (h11-h21)/h11 and (h11-h22)/h11

    To acces the results involving h10 in the calculation, run:
        comp[h10]

    To further filter the results and get a list of objects involving h10 and
    h21 together in the calculation, run:
        comp[h10, h21]

    To get the unfiltered result run:
        comp.results
    ...
    def __init__(self, plotables, func=None, errfunc=None, keytoclone=None):
        ...
        for item in self.symfunc.free_symbols:
            ...
            if item[0] == 'd':
                ...
                raise KeyError(("{0} has an incorrect form. Value from"
                    " values from plotables should be indicated as xN, errors - as dxN (N=0,1,2,...)".format(item)))
        ...
        if self.errfunc is not None:
            ...
            for item in self.symfunc.free_symbols:
                if str(item)[0] == 'd':
                    #If errors from plotables are used in func, treat them as a
                    #const term during error calculation, i.e. error has no
                    #error itself
                    d = 0
                else:
            ...
            for i, ibin in enumerate(result):
                ...
                for key in self.symkeys:
                    if str(key)[0] != 'd':
                        ...
                        subs_func[key] = comb[str(key)[1:]][i].error
            ...
            for i, point in enumerate(result):
                ...
                for key in self.symkeys:
                    if str(key)[0] != 'd':
                        ...
                        subs_func[key] = 0.5*(comb[str(key)[1:]][i].y.error_low+comb[str(key)[1:]][i].y.error_hi)
                        log.warning(
                                "Errors from Graph are used in func calculation, considering mean of low and hi errors")
                for key in self.errfunc.free_symbols:
                    if str(key)[0] != 'd':
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item,tuple) or isinstance(item,list):
            return [obj for obj in self.results if set(item).issubset(set(obj.parents))]
        return [obj for obj in self.results if item in obj.parents]
